...
<div class="col-md-8">
  ...
  if($item['it_of'] != NULL){
    ...
    $offerVal = $stmt->fetch();
    echo '<div class="col-4">' . number_format((float)($item['it_in'] * 1.22 * ($offerVal[0] / 100 + 1)),2,'.',''); '</div>';
  }?>
  </div>
    <div class="row">
    <p id="itemDesc" class="col-8"><?php echo $item['it_ds']; ?></p>
      <form method="POST" action="confirm.php" class="col-4">
      <button class="btn btn-warning mainCardButton" type="submit" name="sale" value="<?php echo $item['it_id'];?>">
    ...
    <form action="modules/rating.php" method="post">
      <fieldset id="calificacion" class="row">
        ...
        ?>
        <input
        class="rating rating--nojs bg-blue col-9"
        style="width:16.3em"
        max="5"
        name="inCal"
        step=".5"
        ...
        />
        <button type="submit" class="btn btn-info col-3" name="id" value="<?php echo $item['it_id'];?>">¡Calificar!</button>
      </fieldset>
    </form>
    </div>
    </div>
